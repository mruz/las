<h3><span class="glyphicon glyphicon-envelope"></span> {{ __('Messages') }} / {{ __(action|capitalize) }}</h3><hr />
{{ flashSession.output() }}
...
<div class="form-group{{ errors is defined and errors.filter(field) ? ' has-error' : (_POST[field]|isset ? ' has-success' : '') }}">
    ...
    <div class="col-lg-9">
        {% if action == 'add' %}
            {{ select(field, clients, 'using': ['id', 'fullName'], 'class': 'form-control', 'useEmpty': true, 'emptyValue': 0, 'emptyText': __('To all')) }}
        {% else %}
            {{ select(field, clients, 'using': ['id', 'fullName'], 'class': 'form-control') }}
        {% endif %}
        {% if errors is defined and errors.filter(field) %}
            <span class="help-block">{{ current(errors.filter(field)).getMessage() }}</span>
        {% endif %}
