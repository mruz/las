...
<html lang="{{ substr(i18n.lang(), 0, 2) }}">
    ...
    <body>
        <div id="wrap">
            ...
            <div class="container">
                <ol class="breadcrumb">
                    <li {{ controller == 'index' and action == 'index' ? ' class="active"' : ''}}>{{ linkTo('admin', __('Admin panel')) }}</li>
                    {% if controller != 'index' %}
                        {% if action == 'index' %}
                            <li class="active">{{ linkTo('admin/' ~ controller, __(controller|capitalize)) }}</li>
                        {% else %}
                            <li>{{ linkTo('admin/' ~ controller, __(controller|capitalize)) }}</li>
                            <li class="active">{{ linkTo('admin/' ~ controller ~ '/' ~ action, __(action|capitalize)) }}</li>
                        {% endif %}
                    {% endif %}
                </ol>
                <div class="row">
