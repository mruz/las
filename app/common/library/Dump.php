...
abstract class Dump
{
    ...
    public static $style = array(
        'pre' => array('background-color' => '#f1f1f1', 'font-size' => '11px', /*'white-space' => 'normal',*/ 'padding' => '10px', 'border' => '1px solid #ccc', 'text-align' => 'left', 'color' => '#222'),
    ...
    public static function all()
    {
        ...
        foreach ($vars as $index => $value) {
            $out .= self::one($value, 'var ' . ($index + 1));
        }
        return $out;
    }
    ...
    private static function _get_info_var($var, $name = '', $indent = 0)
    {
        ...
        $indentChars = '  ';
        $spc = $indent > 0 ? str_repeat($indentChars, $indent) : '';
            ...
            if (in_array($class, $methods)) {
                ...
                while (list($prop, $val) = each($arr)) {
                    if ($val != $class && $val != '__construct') {
                        $out .= $indentChars . $spc . '  ' . '-><span style="' . $style['_obj'] . '">' . $val . "</span>();\n";
                    } else {
                        $out .= $indentChars . $spc . '  ' . "->$val(); [<b>constructor</b>]\n";
                    }
                }
                $out .= "$spc  " . ")\n";
            }
