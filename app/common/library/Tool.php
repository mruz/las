...
class Tool
{
    ...
    public static function registerEngines($view, $di)
    {
        ...
        return array(
            ...
            ".volt" => $volt,
            ".md" => 'Las\Extension\Markdown',
        );
    }

    public static function sortLink($type, $ico = 'order')
    {
        $request = \Phalcon\DI::getDefault()->getShared('request');
        $query = $request->getQuery();
        $url = substr($query['_url'], 1);
        unset($query['_url'], $query['page']);

        if ($request->getQuery('order') == $type) {
            $query ['order'] = $type . ' DESC';
            $ico = '<span class="glyphicon glyphicon-sort-by-' . $ico . '-alt"></span>';
        } else {
            $query ['order'] = $type;
            $ico = '<span class="glyphicon glyphicon-sort-by-' . $ico . '"></span>';
        }
